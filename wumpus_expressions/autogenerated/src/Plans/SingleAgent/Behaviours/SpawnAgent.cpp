...
namespace alica
{
            ...
            DomainBehaviour("SpawnAgent")
    {
        ...
        this->multiAgentSpawnResponseSub = n.subscribe("wumpus_simulator/SpawnMultiAgentResponse", 10,
                                                       &SpawnAgent::onMultiInitialPoseResponse, (SpawnAgent*)this);
    ...
    void SpawnAgent::run(void* msg)
    {
        ...
        if (this->loadedInitialWorld <= 10)
        {
            ...
            loadWorldRequest.worldPath = std::string(getenv("HOME")) + "/" + this->wm->experiment->testRunDirectory
                    + "/" + request;
...
//        std::cout << "sending load world request 1 " << std::endl;
        ...
        auto registered = this->wm->experiment->getCurrentRun()->spawnRequestRegistered(
                this->wm->experiment->getCurrentRun()->getCurrentStartPositionsEncoding());
        if (!registered)
        {
            this->sendMultiInitialPoseRequest();
            this->wm->experiment->getCurrentRun()->registerSpawnRequest(
                    this->wm->experiment->getCurrentRun()->getCurrentStartPositionsEncoding());
            return;
        }

        auto status = this->wm->experiment->getCurrentRun()->getSpawnRequestStatus(
                this->wm->experiment->getCurrentRun()->getCurrentStartPositionsEncoding());
...
//        std::cout << "Sending load World request2 " << std::endl;
    ...
    void SpawnAgent::onMultiInitialPoseResponse(wumpus_simulator::MultiInitialPoseResponsePtr response)
    {
        if (!response->success)
        {
            ...
            //        if (this->spawnRequestSuccessful.find(this->wm->experiment->getCurrentRun()->getCurrentStartPositionsEncoding()) !=
            //                this->spawnRequestSuccessful.end()) {
            //            //this->spawnRequestSuccessful.at(this->wm->experiment->getCurrentRun()->getCurrentStartPositionsEncoding()) = false;
            //
            //        } else {
            //            this->spawnRequestSuccessful.emplace(this->wm->experiment->getCurrentRun()->getCurrentStartPositionsEncoding(), false);
            //        }
            this->wm->experiment->getCurrentRun()->setSpawnRequestStatus(
                    this->wm->experiment->getCurrentRun()->getCurrentStartPositionsEncoding(),
                    eval::SpawnRequestStatus::FAILED);
        }
        else
        {
            this->wm->experiment->getCurrentRun()->setSpawnRequestStatus(
                    this->wm->experiment->getCurrentRun()->getCurrentStartPositionsEncoding(),
                    eval::SpawnRequestStatus::ACCEPTED);
        }
    }
